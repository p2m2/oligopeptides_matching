...
for (i in 1:n){
  for (j in 1:m){
    combined_compounds_names[k] <-
      paste(names(compounds_1)[i], names(compounds_2)[j], sep = "_")
    combined_compounds_mass[k] <-
      compounds_1[i] + compounds_2[j] - chemical_reaction
    k <- k+1
  }
}

combined_compounds <-
  (stats::setNames(combined_compounds_mass, combined_compounds_names))
combined_compounds <-
  combined_compounds[unique(names(combined_compounds))]
