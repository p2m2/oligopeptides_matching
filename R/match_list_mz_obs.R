...
match_list_mz_obs <- function(list_mz_obs,  # contient les mz, name, rt
  ...
  for (i in 1:nrow(list_mz_obs)) {
    feature_name <- list_mz_obs[i, "features_name"]
    mz <- list_mz_obs[i, "mz"]
    rt <- list_mz_obs[i, "rt"]
    
    match_mass <- match_mz_obs(mz_obs, ionization, combined_compounds, ppm_error)
    if (nrow(match_mass) > 0) {
      match_mass <- cbind(feature_name, rt, match_mass)
    } else {
      # Si aucun match n'est trouvé, ajouter une ligne avec NA pour les colonnes de match
      match_mass <- data.frame(feature_name = feature_name, rt = rt,mz = NA, mz_obs = mz_obs, mass = NA, ppm_error_value = NA)
    }
    
    results <- rbind(results, match_mass)
  }
  results <- dplyr::select(results, feature_name, rt, mz, mz_obs, mass, ppm_error_value)
  return(results)
}  